...
class NetworkControllerTest {
    ...
    public void findAllMembers() throws Exception {
        // Given
        Member member1 = memberRepository.save(Member.googleBuilder()
                .googleEmail("abc@abc.com")
                .name("김슈니")
                .profileImagePath("path")
                .build());

        Member member2 = memberRepository.save(Member.googleBuilder()
        ...
        mockMvc.perform(post("/api/v1/network")
    ...
    public void findFilteredMembers() throws Exception {
        ...
        // 예시로 몇 가지 값을 할당
        //final List<String> departments = Arrays.asList("Server","Android");
        //final List<String> parts = Arrays.asList("Back");
        final List<String> levels = Arrays.asList("Member");
    ...
    public void setBookmark() throws Exception {
        ...
        // then
        mockMvc.perform(post("/api/v1/network/bookmark/" + memberId)
                        .contentType(APPLICATION_JSON).header("Authorization", "Bearer " + loginResponseDto.getAccessToken()))
                .andExpect(status().isOk())
                .andDo(print());
    }
